...
cfg.param_defs = { ...
  ...
  struct('name','kcat_braf_act','prior','point','value',log10(1.6059e6),'units','?'), ... 
  struct('name','kf_mtorc2_akt','prior','point','value',log10(6.4236e6),'units','?'), ... 
  struct('name','kr_mtorc2_akt','prior','uniform','min',log10(1e4),'max',log10(1e5),'units','?'), ... 
  struct('name','kphos_s473','prior','uniform','min',log10(1e4),'max',log10(5e5),'units','?'), ... 
  struct('name','kf_egfr_grb2_bind','prior','uniform','min',log10(1e4),'max',log10(5e5),'units','?'), ... 
  struct('name','kr_egfr_grb2_unbind','prior','uniform','min',log10(1e4),'max',log10(5e5),'units','?'), ...
  struct('name','kf_egf_egfr_bind','prior','point','value',log10(1),'units','?'), ... 
  struct('name','kr_egf_egfr_bind','prior','point','value',log10(1),'units','?'), ... 
  struct('name','kf_egfr_akt','prior','uniform','min',log10(1e4),'max',log10(5e5),'units','?'), ... 
  struct('name','kr_egfr_akt','prior','uniform','min',log10(1e4),'max',log10(5e5),'units','?'), ... 
  struct('name','kc_egfr_akt','prior','uniform','min',log10(1e4),'max',log10(5e5),'units','?'), ... 
  struct('name','kf_akt_raf1','prior','uniform','min',log10(1e4),'max',log10(5e5),'units','?'), ... 
  struct('name','kr_akt_raf1','prior','uniform','min',log10(1e4),'max',log10(5e5),'units','?'), ... 
  struct('name','kc_akt_raf1','prior','uniform','min',log10(1e4),'max',log10(5e5),'units','?'), ... 
  struct('name','kf_raf1_map2k1','prior','point','value',log10(2),'units','?'), ... 
  struct('name','kc__raf1_map2k1','prior','uniform','min',log10(1e-10),'max',log10(1e-7),'units','?'), ... 
  struct('name','kt_raf1','prior','uniform','min',log10(1e-3),'max',log10(1e-1),'units','?'), ... 
  struct('name','kf_map2k1_mapk3','prior','point','value',log10(0),'units','?'), ... 
  struct('name','kc_map2k1_mapk3','prior','uniform','min',log10(1e-9),'max',log10(5e-7),'units','?'), ... 
  struct('name','kr_map2k1_mapk3','prior','uniform','min',log10(1e-3),'max',log10(1e-1),'units','?'), ... 
  ...
  struct('name','kf_mapk1_mapk3','prior','uniform','min',log10(1e-9),'max',log10(1e-7),'units','?'), ... 
  struct('name','kr_mapk1_mapk3','prior','uniform','min',log10(1e-3),'max',log10(1e-1),'units','?'), ... 
  struct('name','kf_mtor_rictor','prior','uniform','min',log10(1e-5),'max',log10(1e-1),'units','?'), ... 
  ...
  struct('name','kr_mtor_rptor','prior','uniform','min',log10(1e-4),'max',log10(1e-2),'units','?'), ... 
  struct('name','kf_egfr_rictor','prior','uniform','min',log10(1e-1),'max',log10(1e1),'units','?'), ... 
  struct('name','kc_egfr_rictor','prior','uniform','min',log10(1e-6),'max',log10(1e-4),'units','?'), ... 
  struct('name','kr__egfr_rictor','prior','uniform','min',log10(1e-4),'max',log10(1e-2),'units','?'), ... 
  struct('name','kf_mtor_s6k','prior','uniform','min',log10(1e-2),'max',log10(1),'units','?'), ... 
  struct('name','kc__mtor_s6k','prior','uniform','min',log10(1e-7),'max',log10(1e-5),'units','?'), ... 
  ...
  struct('name','kc_rictor_s6k','prior','uniform','min',log10(1e-2),'max',log10(1),'units','?'), ... 
  struct('name','kf_s6k_s6','prior','uniform','min',log10(1e-4),'max',log10(1e-2),'units','?'), ... 
  struct('name','kc_s6k_s6','prior','uniform','min',log10(1e-1),'max',log10(1e1),'units','?'), ... 
  struct('name','kr_s6k_s6','prior','uniform','min',log10(1e-7),'max',log10(1e-5),'units','?'), ... 
  struct('name','kf_akt_cdkn1b','prior','uniform','min',log10(1e-3),'max',log10(1e-1),'units','?'), ... 
  struct('name','kc_akt_cdkn1b','prior','uniform','min',log10(1e-2),'max',log10(1),'units','?'), ... 
  struct('name','kr_akt_cdkn1b','prior','uniform','min',log10(1e-4),'max',log10(1e-1),'units','?'), ... 
  struct('name','kf_grb2_sos','prior','uniform','min',log10(1e-6),'max',log10(1e-4),'units','?'), ... 
  struct('name','kr_grb2_sos','prior','uniform','min',log10(1e-3),'max',log10(1),'units','?'), ...
  struct('name','kf_kras_sos','prior','uniform','min',log10(5e-3),'max',log10(1),'units','?'), ... 
  struct('name','kr__kras_sos','prior','uniform','min',log10(1e-7),'max',log10(5e-5),'units','?'), ... 
  struct('name','kf_kras_braf','prior','uniform','min',log10(1e-3),'max',log10(5e-1),'units','?'), ... 
  struct('name','kr_kras_braf','prior','uniform','min',log10(1e-1),'max',log10(1e1),'units','?'), ... 
  struct('name','kc_kras_braf','prior','uniform','min',log10(1e-3),'max',log10(1e-1),'units','?'), ... 
  struct('name','kf_braf_map2k1','prior','uniform','min',log10(1e-5),'max',log10(1e-2),'units','?'), ... 
  struct('name','kr_braf_map2k1','prior','uniform','min',log10(1e-4),'max',log10(1e-1),'units','?'), ... 
  struct('name','kc_braf_map2k1','prior','uniform','min',log10(1e-1),'max',log10(1e1),'units','?'), ... 
  struct('name','RPTOR_0','prior','uniform','min',log10(1e-3),'max',log10(5),'units','?'), ... 
  struct('name','RAF1_0','prior','uniform','min',log10(1e-7),'max',log10(1e-5),'units','?'), ... 
  struct('name','GRB2_0','prior','uniform','min',log10(1e-3),'max',log10(1),'units','?'), ... 
  struct('name','MAPK1_0','prior','uniform','min',log10(1e-2),'max',log10(1),'units','?'), ... 
  struct('name','SOS1_0','prior','uniform','min',log10(1e-3),'max',log10(1e-1),'units','?'), ... 
  struct('name','MTOR_0','prior','uniform','min',log10(1e-2),'max',log10(1),'units','?'), ... 
  struct('name','SOS_0','prior','uniform','min',log10(5e-1),'max',log10(5e1),'units','?'), ... 
  struct('name','RPS6KB1_0','prior','uniform','min',log10(1e-6),'max',log10(1e-4),'units','?'), ... 
  struct('name','MAPK3_0','prior','uniform','min',log10(1e-3),'max',log10(1e-1),'units','?'), ... 
  struct('name','BRAF_0','prior','uniform','min',log10(1e-3),'max',log10(1e-1),'units','?'), ... 
  struct('name','AKT1_0','prior','uniform','min',log10(1e-4),'max',log10(1e-1),'units','?'), ... 
  struct('name','EGFR_0','prior','uniform','min',log10(1e-8),'max',log10(1e-6),'units','?'), ... 
  struct('name','KRAS_0','prior','uniform','min',log10(1e-5),'max',log10(1e-2),'units','?'), ... 
  struct('name','RICTOR_0','prior','uniform','min',log10(1e-4),'max',log10(1e-2),'units','?'), ... 
  struct('name','MAP2K1_0','prior','uniform','min',log10(1e-5),'max',log10(1e-3),'units','?'), ... 
  struct('name','CDKN1B_0','prior','uniform','min',log10(1e-3),'max',log10(5e-1),'units','?'), ... 
  struct('name','RPS6_0','prior','uniform','min',log10(1),'max',log10(5e1),'units','?'), ... 
  struct('name','EGF_0','prior','uniform','min',log10(1e4),'max',log10(5e5),'units','?'), ... 
...
cfg.obsv_defs = { ...
  struct('name','pten', 'units','log10'), ...
  struct('name','pten_mrna', 'units','log10'), ...
  struct('name','akt_p308', 'units','log10'), ...
  struct('name','akt_p473', 'units','log10'), ...
  struct('name','foxo_p', 'units','log10'), ...
  struct('name','total_foxo', 'units','log10'), ...
  struct('name','active_foxo', 'units','log10'), ...
  struct('name','total_akt', 'units','log10'), ...
};
...
cfg.sim_tstop  = 50000;          % simulation stop time, time units (seconds)
cfg.sim_dt     = 25;         % time step for trajectory, time units (seconds)
...
% cfg.data{1}.time = pten_lowdose(1:9,1);
% cfg.data{2}.time = pten_lowdose(1:9,1);
...
cfg.jobname = 'Tcell_pt_v25_no308';            % job name, for file input/output
cfg.shuffle  = 1;                      % shuffle random number streams (seed by clock)
cfg.parallel = 1;                      % parallel? true/false
cfg.maxlabs  = 8;                      % maximum number of labs for parallel computing
cfg.nchains  = 8;                      % number of chains
...
cfg.save_progress_interval = 50;      % How often to save progress 
...
cfg.energy_init_max = 5000;             % maximum allowed energy for initialization
...
cfg.data{1}.protocol_fcn = @(t,init,params) simulate_tcell_v26(t,init,params,cfg.data{1}.dose,cfg); 
...
cfg.equilibrate_fcn = @(t,init,params) simulate_tcell_v26(t,init,params,cfg.data{1}.dose,cfg);
